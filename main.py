...
def get_tbs(bduss):
    ...
    try:
        ...
        logger.error(f"获取tbs出错 {e.__class__}")
...
def get_favorite(bduss):
    ...
    try:
        ...
        logger.error(f"获取关注的贴吧出错 {e.__class__}")
        return []
    return_data = res
...
def encode_data(data):
    ...
    for i in sorted(keys):
        result += i + EQUAL + str(data[i])
    sign = hashlib.md5((result + SIGN_KEY).encode(UTF8)).hexdigest().upper()
